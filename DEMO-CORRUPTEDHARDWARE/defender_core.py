...
def receive_payload(port: int = 9999) -> str:
    ...
    dropped_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dropped_payload.py")

    print(f"[defender] Waiting for payload on 0.0.0.0:{port} ...")
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        with conn:
            print(f"[defender] Connection from {addr[0]}:{addr[1]}")
            chunks = []
            while True:
                data = conn.recv(4096)
                if not data:
    ...
    if not payload:
        raise RuntimeError("Received empty payload on port 9999")

    with open(dropped_path, "wb") as f:
        f.write(payload)

    print(f"[defender] Payload saved to: {dropped_path}")
    return dropped_path
...
class Defender:
    def __init__(self, python_exe: str, script_path: str | None = None):
        self.python_exe  = python_exe
        self.script_path = script_path or receive_payload()
        self.pid         = None
        self.session     = None
        self.script      = None
...
def main():
    ...
    ap.add_argument("--script", "-s", default=None,
                    help="Path to script to run. If omitted, waits for network payload on port 9999.")
    args = ap.parse_args()
